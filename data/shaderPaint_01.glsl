...
void main(void) {
    // canvas doesn't match screen size
    ...
    float clouds = fbm(gl_FragCoord.xy * 0.0015115 + vec2( sin(time * 0.0514739)*.5, cos(time * 0.041297471)*.5 ) );
    float cloudsTime = fbm(gl_FragCoord.xy * 0.0012317 + vec2( cos(time * 0.0214739)*.5, sin(time * 0.07197471)*.5 ) );

    vec2 vStretch = vec2( gl_FragCoord.x * 0.25, gl_FragCoord.y );
    float v = (snoise(
        vec3(vStretch * 0.000751417, (sin(time * 0.141545) + cos(time * .15321)) * cloudsTime * .5 ) +
        vec3(vStretch * 0.000613514123, (cos(time * 0.131411) + sin(time * .14762)) * cloudsTime * .5 )
    ) + 1.0) * 0.75;

    v *= (snoise(
        vec3(vStretch * 0.0007643, cos(time * 0.0147) * v) +
        vec3(vStretch * 0.000321, sin(time * 0.014401) * v)
    ) + 1.0) * 0.75;

    v *= clouds * 0.25 + 0.5; // modulate brightness with clouds
    ...
    float h = snoise(
        vec3(hStretch * 0.00178431 * hMod * v , sin(time * 0.025) + (cloudsTime  * .041) ) +
        vec3(hStretch * 0.004721 * hMod * v, cos(time * 0.01) + (cloudsTime * .051)) +
    ...
    rgb = brightnessContrast(rgb, 0.0, 2.05); // adjust brightness and contrast
