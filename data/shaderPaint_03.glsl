...
void main(void) {
    ...
    // lumin modulation
    // float luminMod = fbm(gl_FragCoord.xy * 0.00512317 + time *sin(lumin*PI*.09 + time*.00001)* 0.7 +lumin * 0.1); ;
    // lumin *= luminMod; // modulate luminance with noise

    // brightness

    float clouds = fbm(gl_FragCoord.xy * 0.0015115 + vec2( sin(time * 0.0514739)*.5 + lumin * .044, cos(time * 0.141297471)*.5 ) * lumin  );
    float cloudsTime = fbm(gl_FragCoord.xy * 0.0012317 + vec2( cos(time * 0.0214739)*.5 + lumin * .077, sin(time * 0.17197471)*.5 ) * lumin  );
